...
//	moneyBucket4.fill(0);
	...
	*/
	...
	function resetPnB(){
		...
		buildings = [new building("Shoes", 20, 1.069, 1), new building("Spade", 200, 1.053, 2), new building("Bingo", 2300, 1.11, 19), new building("Spaceship", 9000, 1.19, player.shipBaseGain), new building("Magic", 50000, 1.02, 30), new building("Gold Bat 9000", 999999, 1.24, 9999), new building("Ice Cubes", 70000000, 1.85, 420), new building("Garbage", 654321000, 1.15, 1000), new building("Pie", 3141592653589, 1.214, 31415926), new building("Sleeping Giant", 9999999999999,1.3,1100000000), new building("Red Paper Clip",398536512698354,1.2,1)];
	}
	...
	function pageRefresh(){
		...
		document.getElementById("baterestValue2").innerHTML = (Math.round((buildings[5].amount/baterestGain + .2) * 100));
		...
		document.getElementById("totalmoneySpan").innerHTML = shortenLargeNumber(totalMoney);
		document.getElementById("spendOnShoe").innerHTML = shortenLargeNumber(moneySpentPerBuilding[0]);
		document.getElementById("spendOnSpade").innerHTML = shortenLargeNumber(moneySpentPerBuilding[1]);
		document.getElementById("spendOnBingo").innerHTML = shortenLargeNumber(moneySpentPerBuilding[2]);
		document.getElementById("spendOnShip").innerHTML = shortenLargeNumber(moneySpentPerBuilding[3]);
		document.getElementById("spendOnMagic").innerHTML = shortenLargeNumber(moneySpentPerBuilding[4]);
		document.getElementById("spendOnBat").innerHTML = shortenLargeNumber(moneySpentPerBuilding[5]);
		document.getElementById("spendOnCube").innerHTML = shortenLargeNumber(moneySpentPerBuilding[6]);
		document.getElementById("spendOnGarbage").innerHTML = shortenLargeNumber(moneySpentPerBuilding[7]);
...
//		$('#building1Upgrade').css({"background-image": "url(art/build1.png)", "background-size":"contain"});
	...
	function save(){
		...
		localStorage.setItem('winCondition', youWon);
	...
	function load(){
			...
			youWon = (localStorage.getItem('winCondition') == 'true');
		...
		saveLoadPopup();
	//	handleKongStatSubmit();
	}
	function handleKongStatSubmit(){
			if(youWon){
				parent.kongregate.stats.submit('You Win!', 1);
				parent.kongregate.stats.submit('Completion Time', totalTicks);
			}
			parent.kongregate.stats.submit('Buildings Owned', getTotalBuildingCount());
